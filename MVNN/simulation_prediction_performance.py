...
def evaluate_network(
    ...
    return eval_config(
        ...
        bidder_id=bidder_type_to_bidder_id[SAT_instance.upper()][bidder_type],
        layer_type=layer_type,
        batch_size=cfg["batch_size"],
        num_hidden_layers=cfg["num_hidden_layers"],
        num_hidden_units=int(
            max(1, np.round(cfg["num_neurons"] / cfg["num_hidden_layers"]))
        ),
        l2=cfg["l2"],
        lr=cfg["lr"],
...
def main():
    ...
    config_dict = hpo_results[args.domain][str(args.T)][args.bidder_type][
        network_type_to_layer_type[args.network_type]
    ]
    print("Selected winner hyperparameters", config_dict)
    model, logs = evaluate_network(
        config_dict,
        seed=args.seed,
        SAT_instance=args.domain,
        bidder_type=args.bidder_type,
        num_train_data=args.T,
    ...
    print(
        "Train metrics \t| pearson corr.: {:.3f}, KT: {:.3f}".format(
            train_logs["r"], train_logs["kendall_tau"]
        )
    )
    print(
        "Valid metrics \t| pearson corr.: {:.3f}, KT: {:.3f}".format(
            val_logs["r"], val_logs["kendall_tau"]
        )
    )
    print(
        "Test metrics \t| pearson corr.: {:.3f}, KT: {:.3f}".format(
            test_logs["r"], test_logs["kendall_tau"]
        )
    )
