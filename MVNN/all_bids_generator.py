...
parser.add_argument(
    "--number_of_instances", type=int, default=1, help="Num. training data"
)

args = parser.parse_args()
path = "data/" + args.domain + "/"
seeds_instances = list(range(1, 1 + args.number_of_instances)) + [
    127
]  # for instances in (1)
...
def eval_seed(model_name, seed):
    ...
    bundle_value_pairs = generate_all_bundle_value_pairs(
        value_model, order=0 if model_name not in ["SRVM", "MRVM"] else 2
    )
...
eval_seed(model_name=args.domain, seed=157)
