...
def mlca_mechanism(
    ...
    if init_bids is not None:
        E.set_initial_bids(
            initial_bids=init_bids, fitted_scaler=init_fitted_scaler
        )  # (*) use self defined inital bids | Line 1
    else:
        E.set_initial_bids(
            seed=SATS_auction_instance_seed
        )  # create inital bids B^0, uniformly sampling at random from bundle space | Line 2 (now depending on isLegacy with correct sampling)
    ...
    while CHECK:
        ...
        for bidder in E.bidder_names:
            ...
            sampled_marginal_economies = E.sample_marginal_economies(
                active_bidder=key_to_int(bidder), number_of_marginals=E.Qround - 1
            )
    ...
    logging.warning(
        "NORMAL OPTIMIZATIONS: {}/ BIDDER SPECIFIC: {}".format(
            E.number_of_optimization["normal"],
            E.number_of_optimization["bidder_specific"],
        )
    )
    logging.warning(
        "EFFICIENCY: {} %".format(round(E.mlca_allocation_efficiency, 4) * 100)
    )
    logging.warning(
        "TOTAL TIME ELAPSED: {}".format("{}d {}h:{}m:{}s".format(*total_time_elapsed))
    )
