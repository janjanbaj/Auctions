...
class MLCA_WDP:
    def __init__(self, bids):
        ...
        self.x_star = np.zeros(
            (self.N, self.M), dtype=int
        )  # optimal allocation of the winner determination problem
    ...
    def initialize_mip(self, verbose=0):
        ...
        for i in range(0, self.N):  # over bidders i \in N
            ...
            self.z.update(
                {
                    (i, k): self.Mip.binary_var(name="z({},{})".format(i, k))
                    for k in range(0, self.K[i])
                }
            )  # z(i,k) = 1 <=> bidder i gets bundle k \in Ki
            # add allocation constraints for z(i,k)
            self.Mip.add_constraint(
                ct=(self.Mip.sum(self.z[(i, k)] for k in range(self.K[i])) <= 1),
        ...
        objective = self.Mip.sum(
            self.z[(i, k)] * self.bids[i][k, self.M]
            for i in range(0, self.N)
            for k in range(0, self.K[i])
        )
        ...
        for i in range(0, self.N):
            for k in range(0, self.K[i]):
                if self.z[(i, k)].solution_value != 0:
                    self.x_star[i, :] = (
                        self.z[(i, k)].solution_value * self.bids[i][k, :-1]
                    )
        ...
        try:
            for i in range(0, self.N):
                for k in range(0, self.K[i]):
                    if self.z[(i, k)].solution_value != 0:
                        print(
                            "z({},{})=".format(i, k), int(self.z[(i, k)].solution_value)
                        )
        except Exception:
            print("Not yet solved!\n")
        print(
