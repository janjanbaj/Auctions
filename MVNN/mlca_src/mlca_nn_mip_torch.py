...
import mlca_src.gurobi_wrapper as cpx
...
class NN_MIP_TORCH:
    ...
    def __init__(self, models, L=None):
        ...
        self.M = list(models[list(models.keys())[0]].parameters())[0].shape[
            1
        ]  # number of items in the value model = dimension of input layer
        ...
        self.Mip = cpx.Model(
            name="NeuralNetworksMixedIntegerProgram"
        )  # docplex instance
        ...
        self.x_star = np.ones(shape=(self.N, self.M), dtype=int) * (
            -1
        )  # optimal allocation (-1=not yet solved)
                    ...
                    (
                        bidder_name,
                        {
                            ...
                            for idx, val in enumerate(
                                np.array(
                                    [None] * layer.shape[0], dtype=np.float32
                                ).reshape(-1, 1)
                                for layer in self._get_model_layers(
                                    bidder_name, exc_layer_type=["output"]
                                )
                            )
                        },
                    )
                ...
                list(
                    (
                        bidder_name,
                        {
                            idx + 1: val
                            for idx, val in enumerate(
                                np.array([True] * layer.shape[0]).reshape(-1, 1)
                                for layer in self._get_model_layers(
                                    bidder_name, exc_layer_type=["output"]
                                )
                            )
                ...
                list(
                    (
                        bidder_name,
                        {
                            ...
                            for idx, val in enumerate(
                                np.array(
                                    [self.L] * layer.shape[0], dtype=np.float32
                                ).reshape(-1, 1)
                                for layer in self._get_model_layers(
                                    bidder_name, exc_layer_type=["output"]
                                )
                    ...
                    (
                        bidder_name,
                        {
                            ...
                            for idx, val in enumerate(
                                ...
                                for layer in self._get_model_layers(bidder_name)
                            )
                        },
                    )
                    for bidder_name in self.sorted_bidders
    ...
    def solve_mip(
        ...
        logging.info(
            "MIP relative gap %s", self.Mip.parameters.mip.tolerances.mipgap.get()
        )
        logging.info(
            "MIP integrality tol %s",
            self.Mip.parameters.mip.tolerances.integrality.get(),
        )
    ...
    def log_solve_details(self, solved_mip):
        ...
        if self.NN_type == "UNN":
            logging.info(
                f" - number of UNN-activation-function-constraints removed: {self.removed_activation_cts}"
            )
        else:
            logging.info(
        ...
        return {
            ...
            "rel. gap": details.mip_relative_gap,
            "hit_limit": details.has_hit_limit(),
            "time": details.time,
        }
    ...
    def _get_model_weights(self, key, layer_type=["layers"]):
        model_weights = []
            ...
            if any(weight_desc[0] in layer for layer in layer_type):
                if not ("bias" in k or "weight" in k):
                    raise NotImplementedError(
                        "Unknown weight type encountered: {}".format(k)
                    )
                else:
                    model_weights.append(v.detach().cpu().numpy())
        return model_weights

    def _get_model_layers(self, key, exc_layer_type=[]):
        layers = []
        for k, v in dict(self.Models[key].named_parameters()).items():
            if "bias" not in k:
    ...
    def _clean_weights(self, Wb):
        for v in range(0, len(Wb) - 2, 2):
            ...
            zero_rows = np.where(
                np.logical_and((Wb[v] == 0).all(axis=0), Wb[v + 1] == 0)
            )[0]
            if len(zero_rows) > 0:
    ...
    def _add_matrix_constraints(self, i, verbose=False):
        ...
        if self.NN_type == "UNN":
            ...
            assert len(Wb) % 2 == 0, (
                "Currently weights and biases should always be coupled."
            )
            ...
            for v in range(0, len(Wb), 2):  # loop over layers
                ...
                if v == 0:
                    self.z.update(
                        {
                            (i, 0, j): self.Mip.binary_var(name="x({})_{}".format(i, j))
                            for j in range(0, J)
                        }
                    )  # binary variables for allocation
                self.z.update(
                    {
                        (i, layer, r): self.Mip.continuous_var(
                            name="z({},{})_{}".format(i, layer, r)
                        )
                        for r in range(0, R)
                    }
                )  # output value variables after activation
                ...
                for r in range(0, R):
                    ...
                    # Case 2: "for sure in the linear range of the UNN-activation function" (given IA Bounds on Neurons)
                        ...
                        self.upper_bounds_z[key][layer][r][0] > 0
                        and self.upper_bounds_z[key][layer][r][0] <= ts[layer - 1]
                        and self.lower_bounds_z[key][layer][r][0] < ts[layer - 1]
                        and self.lower_bounds_z[key][layer][r][0] >= 0
                    ):
                        self.active[key][layer][r][0] = False
                ...
                self.y.update(
                    {
                        (i, layer, r): self.Mip.binary_var(
                            name="y({},{})_{}".format(i, layer, r)
                        )
                        for r in range(0, R)
                        if self.active[key][layer][r][0]
                    }
                )  # binary variables for activation function
                self.eta.update(
                    {
                        (i, layer, r): self.Mip.continuous_var(
                            name="eta({},{})_{}".format(i, layer, r)
                        )
                        for r in range(0, R)
                        if self.active[key][layer][r][0]
                    }
                ...
                for r in range(0, R):
                    if verbose is True:
                        ...
                        logging.debug(
                            "upper (preactivated) z-bound: {}, {}, {}, {}".format(
                                key, layer, r, self.upper_bounds_z[key][layer][r][0]
                            )
                        )
                        logging.debug(
                            "lower (preactivated) z-bound: {}, {}, {}, {}".format(
                                key, layer, r, self.lower_bounds_z[key][layer][r][0]
                            )
                        )
                        logging.debug(
                            "L1: {}, {}, {}, {}".format(
                                key, layer, r, self.L1[key][layer][r]
                            )
                        )
                        logging.debug(
                            "L2: {}, {}, {}, {}".format(
                                key, layer, r, self.L2[key][layer][r]
                            )
                        )
                        logging.debug(
                            "L3: {}, {}, {}, {}".format(
                                key, layer, r, self.L3[key][layer][r]
                            )
                        )
                        logging.debug(
                            "L4: {}, {}, {}, {}".format(
                                key, layer, r, self.L4[key][layer][r]
                            )
                        )
                    ...
                    if self.upper_bounds_z[key][layer][r][0] <= 0:
                        ...
                        self.Mip.add_constraint(
                            ct=self.z[(i, layer, r)] == 0,
                            ctname="Case1_CT_Bidder{}_Layer{}_Row{}".format(
                                i, layer, r
                            ),
                        )
                    ...
                    # Case 2: "for sure in the linear range of the UNN-activation function" (given IA Bounds on Neurons)
                        ...
                        self.Mip.add_constraint(
                            ct=(
                                self.Mip.sum(
                                    W[r, j] * self.z[(i, layer - 1, j)]
                                    for j in range(0, J)
                                )
                                + b[r]
                                == self.z[(i, layer, r)]
                            ),
                            ctname="Case2_CT_Bidder{}_Layer{}_Row{}".format(
                                i, layer, r
                            ),
                        )
                        self.removed_activation_cts += 1
                    ...
                    # Case 4: "non-decidable"
                        ...
                        self.Mip.add_constraint(
                            ...
                            ctname="AffineCT_Bidder{}_Layer{}_Row{}_eta_i_lb".format(
                                i, layer, r
                            ),
                        )
                        self.Mip.add_constraint(
                            ct=(
                                ...
                                + b[r]
                                + self.y[(i, layer, r)] * self.L1[key][layer][r][0]
                            ),
                            ctname="AffineCT_Bidder{}_Layer{}_Row{}_eta_i_ub".format(
                                i, layer, r
                            ),
                        )
                        ...
                        self.Mip.add_constraint(
                            ...
                            ctname="AffineCT_Bidder{}_Layer{}_Row{}_eta_ii_lb".format(
                                i, layer, r
                            ),
                        )
                        self.Mip.add_constraint(
                            ct=(
                                self.eta[(i, layer, r)]
                                <= (1 - self.y[(i, layer, r)])
                                * self.L2[key][layer][r][0]
                            ),
                            ctname="AffineCT_Bidder{}_Layer{}_Row{}_eta_ii_ub".format(
                                i, layer, r
                            ),
                        )
                        ...
                        # (iii.)
                        self.Mip.add_constraint(
                            ct=self.eta[(i, layer, r)]
                            - self.mu[(i, layer, r)] * self.L3[key][layer][r][0]
                            <= self.z[(i, layer, r)],
                            ctname="BinaryCT_Bidder{}_Layer{}_Row{}_z_iii_lb".format(
                                i, layer, r
                            ),
                        )
                        self.Mip.add_constraint(
                            ct=self.z[(i, layer, r)] <= self.eta[(i, layer, r)],
                            ctname="BinaryCT_Bidder{}_Layer{}_Row{}_z_iii_up".format(
                                i, layer, r
                            ),
                        )
                        ...
                        self.Mip.add_constraint(
                            ct=ts[layer - 1]
                            - (1 - self.mu[(i, layer, r)]) * self.L4[key][layer][r][0]
                            <= self.z[(i, layer, r)],
                            ctname="BinaryCT_Bidder{}_Layer{}_Row{}_z_iv_lb".format(
                                i, layer, r
                            ),
                        )
                        self.Mip.add_constraint(
                            ct=self.z[(i, layer, r)] <= ts[layer - 1],
                            ctname="BinaryCT_Bidder{}_Layer{}_Row{}_z_iv_up".format(
                                i, layer, r
                            ),
                        )
                layer += 1
            ...
            self.z.update(
                {
                    (i, layer, r): self.Mip.sum(
                        W[r, j] * self.z[(i, layer - 1, j)] for j in range(0, J)
                    )
                    for r in range(0, R)
                }
            )  # output value variables after activation
        ...
        ###################
            ...
            for v in range(0, len(Wb), 2):  # loop over layers
                ...
                self.z.update(
                    {
                        (i, layer, r): self.Mip.continuous_var(
                            lb=0, name="z({},{})_{}".format(i, layer, r)
                        )
                        for r in range(0, R)
                    }
                )  # output value variables after activation
                self.s.update(
                    {
                        (i, layer, r): self.Mip.continuous_var(
                            lb=0, name="s({},{})_{}".format(i, layer, r)
                        )
                        for r in range(0, R)
                        if (
                            self.upper_bounds_z[key][layer][r][0] != 0
                            and self.upper_bounds_s[key][layer][r][0] != 0
                        )
                    }
                )  # slack variables
                self.y.update(
                ...
                for r in range(0, R):
                    if verbose is True:
                        ...
                        logging.debug("b[r]: %s", b[r])
                        logging.debug(
                            "(preactivated) upper z-bound: {}, {}, {}, {}".format(
                                key, layer, r, self.upper_bounds_z[key][layer][r]
                            )
                        )
                        logging.debug(
                            "(preactivated) upper s-bound: {}, {}, {}, {}".format(
                                key, layer, r, self.upper_bounds_s[key][layer][r]
                            )
                        )
                    ...
                    if self.upper_bounds_z[key][layer][r][0] == 0:
                        if verbose is True:
                            logging.debug(
                                "Case 1: upper z-bound: {}, {}, {} is equal to zero => add z==0 constraints".format(
                                    key, layer, r
                                )
                            )
                        self.Mip.add_constraint(
                    ...
                    # Case 3: "not decidable for sure" (given IA Bounds on Neurons) => need to encode full relu with slack
                        ...
                        self.Mip.add_constraint(
                            ...
                            ctname="AffineCT_Bidder{}_Layer{}_Row{}".format(
                                i, layer, r
                            ),
                        )
                        ...
                        self.Mip.add_constraint(
                            ct=self.z[(i, layer, r)]
                            <= self.y[(i, layer, r)]
                            * self.upper_bounds_z[key][layer][r][0],
                            ctname="BinaryCT_Bidder{}_Layer{}_Row{}_z".format(
                                i, layer, r
                            ),
                        )
                        ...
                        self.Mip.add_constraint(
                            ...
                            * self.upper_bounds_s[key][layer][r][0],
                            ctname="BinaryCT_Bidder{}_Layer{}_Row{}_s".format(
                                i, layer, r
                            ),
                        )
    ...
    def initialize_mip(
        ...
        for j in range(0, self.M):
            self.Mip.add_constraint(
                ct=(self.Mip.sum(self.z[(i, 0, j)] for i in range(0, self.N)) <= 1),
        ...
        if GSVM_specific_constraints and national_circle_complement is not None:
            for i in range(0, self.N):
                ...
                if self.sorted_bidders[i] in [
                    ...
                    logging.debug(
                        "Adding GSVM specific constraints for regional {}.".format(
                            self.sorted_bidders[i]
                        )
                    )
                    self.Mip.add_constraint(
                        ct=(
                            self.Mip.sum(self.z[(i, 0, j)] for j in range(0, self.M))
                            <= 4
                        ),
                ...
                # national bidder
                    ...
                    self.Mip.add_constraint(
                        ct=(
                            self.Mip.sum(
                                self.z[(i, 0, j)] for j in national_circle_complement
                            )
                            == 0
                        ),
        ...
        objective = self.Mip.sum(
            self.Models[self.sorted_bidders[i]]._target_max
            * self.z[(i, self.Models[self.sorted_bidders[i]]._num_hidden_layers + 1, 0)]
            for i in range(0, self.N)
        )
    ...
    def tighten_bounds_LP(self, upper_bound_input, lower_bound_input, verbose=False):
        raise NotImplementedError("LP-tightening not implemented")

    def tighten_bounds_IA(self, upper_bound_input, lower_bound_input, verbose=False):
        logging.info("")
        ...
        if self.NN_type == "UNN":
            ...
            for bidder in self.sorted_bidders:
                ...
                self.upper_bounds_z[bidder][0] = np.array(upper_bound_input).reshape(
                    -1, 1
                )
                self.lower_bounds_z[bidder][0] = np.array(lower_bound_input).reshape(
                    -1, 1
                )
                #######
                ...
                for v in range(0, len(Wb), 2):  # loop over layers
                    ...
                    self.upper_bounds_z[bidder][layer] = W @ phi(
                        self.upper_bounds_z[bidder][layer - 1], ts[layer - 1]
                    ) + b.reshape(-1, 1)  # upper bound for z
                    self.lower_bounds_z[bidder][layer] = W @ phi(
                        self.lower_bounds_z[bidder][layer - 1], ts[layer - 1]
                    ) + b.reshape(-1, 1)  # lower bound for z
                    #####
                    ...
                    for r in range(0, R):
                        ...
                        self.L2[bidder][layer][r][0] = max(
                            0,
                            sum(
                                W[r, j]
                                * phi(
                                    self.upper_bounds_z[bidder][layer - 1][j],
                                    ts[layer - 1],
                                )
                                for j in range(0, J)
                            )
                        ...
                        self.L3[bidder][layer][r][0] = max(
                            0, self.L2[bidder][layer][r][0] - ts[layer - 1]
                        )
            ...
            if verbose is True:
                ...
                for k, v in (
                    pd.DataFrame(
                        {k: pd.Series(l) for k, l in self.upper_bounds_z.items()}
                    )
                    .fillna("-")
                    .items()
                ):
                ...
                for k, v in (
                    pd.DataFrame(
                        {k: pd.Series(l) for k, l in self.lower_bounds_z.items()}
                    )
                    .fillna("-")
                    .items()
                ):
                ...
                logging.debug("L1:")
                for k, v in (
                    pd.DataFrame({k: pd.Series(l) for k, l in self.L1.items()})
                    .fillna("-")
                    .items()
                ):
                    logging.debug(k)
                ...
                logging.debug("L2:")
                for k, v in (
                    pd.DataFrame({k: pd.Series(l) for k, l in self.L2.items()})
                    .fillna("-")
                    .items()
                ):
                    logging.debug(k)
                ...
                logging.debug("L3:")
                for k, v in (
                    pd.DataFrame({k: pd.Series(l) for k, l in self.L3.items()})
                    .fillna("-")
                    .items()
                ):
                    logging.debug(k)
                ...
                logging.debug("L4:")
                for k, v in (
                    pd.DataFrame({k: pd.Series(l) for k, l in self.L4.items()})
                    .fillna("-")
                    .items()
                ):
                    logging.debug(k)
        ...
        ###################
            ...
            for bidder in self.sorted_bidders:
                ...
                Wb = self._get_model_weights(
                    bidder, layer_type=["layers", "output_layer"]
                )
                ...
                self.upper_bounds_s[bidder][0] = np.array(upper_bound_input).reshape(
                    -1, 1
                )
                #######
                ...
                for v in range(0, len(Wb), 2):  # loop over layers
                    ...
                    self.upper_bounds_z[bidder][layer] = np.ceil(
                        np.maximum(
                            W_plus @ self.upper_bounds_z[bidder][layer - 1]
                            + b.reshape(-1, 1),
                            0,
                        )
                    ).astype(int)  # upper bound for z
                    self.upper_bounds_s[bidder][layer] = np.ceil(
                        np.maximum(
                            -(
                                W_minus @ self.upper_bounds_z[bidder][layer - 1]
                                + b.reshape(-1, 1)
                            ),
                            0,
                        )
                    ).astype(int)  # upper bound  for s
                    ######
            ...
            if verbose is True:
                ...
                for k, v in (
                    pd.DataFrame(
                        {k: pd.Series(l) for k, l in self.upper_bounds_s.items()}
                    )
                    .fillna("-")
                    .items()
                ):
    ...
    def print_bounds(self):
        ...
        if self.NN_type == "UNN":
            ...
            for k, v in (
                pd.DataFrame({k: pd.Series(l) for k, l in self.upper_bounds_z.items()})
                .fillna("-")
                .items()
            ):
                print(k)
            ...
            for k, v in (
                pd.DataFrame({k: pd.Series(l) for k, l in self.lower_bounds_z.items()})
                .fillna("-")
                .items()
            ):
                print(k)
        ...
        ###################
            ...
            print("")
            for k, v in (
                pd.DataFrame({k: pd.Series(l) for k, l in self.upper_bounds_s.items()})
                .fillna("-")
                .items()
            ):
