...
def eval_bidder_nn(
    bidder, fitted_scaler, local_scaling_factor, NN_parameters, elicited_bids
):
    ...
    nn_model.initialize_model(
        model_parameters=NN_parameters[bidder]
    )  # initialize model
    train_logs = nn_model.fit(
...
class MLCA_Economies:
    def __init__(
        ...
        self.Qround = (
            Qround  # maximal number of marginal economies per auction round per bidder
        )
        ...
        self.efficiency_per_iteration = (
            OrderedDict()
        )  # storage for efficiency stat püer auction round
        ...
        self.mlca_payments_parts = OrderedDict(
            list(("Bidder_{}".format(bidder_id), None) for bidder_id in self.bidder_ids)
        )
        ...
        )  # Storage for the MAE loss during training of the DNNs
    ...
    def get_info(self, final_summary=False):
        ...
        for k, v in self.elicited_bids.items():
            logging.warning(
                k + ": %s", v[0].shape[0] - 1
            )  # -1 because of added zero bundle
        logging.warning(
    ...
    def get_number_of_elicited_bids(self, bidder=None):
        if bidder is None:
            ...
            )  # -1, because null bundle was added
        else:
    ...
    def calculate_efficiency_per_iteration(self):
        ...
        )  # -1 elicited bids after previous iteration
    ...
    def set_initial_bids(self, initial_bids=None, fitted_scaler=None, seed=None):
        ...
        if initial_bids is None:  # Uniform sampling (now with correct isLegacy Version)
            ...
            if not (list(initial_bids.keys()) == self.bidder_names):
                logging.info(
                    "Cannot set inputed initial bids-> sample uniformly."
                )  # Uniform sampling (now with correct isLegacy Version)
                self.elicited_bids, self.fitted_scaler = util.initial_bids_mlca_unif(
                    SATS_auction_instance=self.SATS_auction_instance,
                    number_initial_bids=self.Qinit,
                    bidder_names=self.bidder_names,
    ...
    def sample_marginal_economies(self, active_bidder, number_of_marginals):
        admissible_marginals = [
            ...
            if x not in ["Main Economy", "Marginal Economy -({})".format(active_bidder)]
        ]
    ...
    def update_elicited_bids(self):
        for bidder in self.bidder_names:
            ...
            bidder_value_reports = self.value_queries(
                bidder_id=util.key_to_int(bidder),
    ...
    def update_current_query_profile(self, bidder, bundle_to_add):
        if bundle_to_add.shape != (self.M,):
            ...
            return False
        if (
            self.current_query_profile[bidder] is None
        ):  # If empty can add bundle_to_add for sure
    ...
    def check_bundle_contained(self, bundle, bidder):
        ...
        if self.current_query_profile[bidder] is not None:
            if np.any(np.equal(self.current_query_profile[bidder], bundle).all(axis=1)):
                logging.info(
                    "Argmax bundle ALREADY QUERIED IN THIS AUCTION ROUND from {}\n".format(
                        bidder
                    )
                )
                return True
    ...
    def next_queries(self, economy_key, active_bidder):
        if not self.economy_status[
            economy_key
        ]:  # check if economy already has been calculated prior
        ...
        if self.check_bundle_contained(
        ...
        ):  # Check if argmax bundle already has been queried in R or S
            ...
            ##########
            if (
                self.current_query_profile[active_bidder] is not None
            ):  # recalc optimization step with bidder specific constraints added
            ...
            allocation = copy.deepcopy(
                self.argmax_allocation[economy_key][active_bidder][1]
            )
    ...
    def estimation_step(self, economy_key):
        ...
        models = merge_dicts(
            *[
                ...
                for key in self.economies_names[economy_key]
            ]
        )
    ...
    def optimization_step(self, economy_key, bidder_specific_constraints=None):
        DNNs = OrderedDict(
            list(
                (key, self.NN_models[economy_key][key].model)
                for key in list(self.NN_models[economy_key].keys())
            )
        )
        ...
        if bidder_specific_constraints is None:
            ...
            logging.info(
                "ADDITIONAL BIDDER SPECIFIC OPTIMIZATION STEP for {}".format(
        ...
        for attempt in range(1, attempts + 1):
            ...
            if not self.MIP_parameters["mip_bounds_tightening"]:
                ...
                X.tighten_bounds_IA(
                    upper_bound_input=[1] * self.M, lower_bound_input=[0] * self.M
                )
                X.initialize_mip(
                    verbose=False,
                    bidder_specific_constraints=bidder_specific_constraints,
            ...
            try:
                ...
                if (
                    ...
                    sol, log = X.solve_mip(
                        ...
                        mip_start=docplex.mp.solution.SolveSolution(
                            X.Mip, self.warm_start_sol[economy_key].as_dict()
                        ),
                    )
                ...
                if bidder_specific_constraints is None:
                    ...
                    logging.debug(
                        "SET ARGMAX ALLOCATION ONLY BIDDER SPECIFIC for {}".format(
                            list(bidder_specific_constraints.keys())[0]
                        )
                    )
                ...
                logging.warning(f"FOR REASON: {e}")
                exception_type, exception_object, exception_traceback = sys.exc_info()
                traceback.print_tb(exception_traceback, limit=3, file=sys.stdout)
    ...
    def solve_WDP(
        self, elicited_bids, verbose=0
    ):  # rem.: objective always rescaled to true values
        ...
        allocation = util.format_solution_mip_new(
            Mip=wdp.Mip,
    ...
    def calculate_vcg_payments(self, forced_recalc=False):
        ...
        logging.info(
            "Revenue: {} | {}% of SCW in efficient allocation\n".format(
                self.revenue, self.relative_revenue
            )
        )
