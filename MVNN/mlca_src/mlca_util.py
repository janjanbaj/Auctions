...
problem_instance_info = {
    "GSVM": instance_info(
        bidder_types=["Regional", "National"], Qmax=100, Qround=4, Qinit=40
    ),
    "LSVM": instance_info(
        bidder_types=["Regional", "National"], Qmax=100, Qround=4, Qinit=40
    ),
    "MRVM": instance_info(
        bidder_types=["Local", "Regional", "National"], Qmax=500, Qround=4, Qinit=40
    ),
    "SRVM": instance_info(
        bidder_types=["Local", "Regional", "National", "High_Frequency"],
        Qmax=100,
        Qround=4,
        Qinit=40,
    ),
...
def initial_bids_mlca_unif(
    SATS_auction_instance, number_initial_bids, bidder_names, scaler=None, seed=None
):
    ...
    for bidder in bidder_names:
        ...
        D = np.asarray(
            SATS_auction_instance.get_uniform_random_bids(
        ...
        seed_additional_bundle = (
            None if bidder_seeds[i] is None else 10**6 * bidder_seeds[i]
        )
        while (
            len(unique_indices) != number_initial_bids + 1
        ):  # +1, because null bundle was already added above in *
            tmp = np.asarray(
                SATS_auction_instance.get_uniform_random_bids(
                    bidder_id=key_to_int(bidder),
                    number_of_bids=1,
            ...
            if seed_additional_bundle is not None:
    ...
    if scaler is not None:
        ...
        logging.debug(
            "Scaling by: %s | %s==feature range max?",
            scaler.scale_,
        ...
        initial_bids = OrderedDict(
            list(
                (key, [value[0], scaler.transform(value[1].reshape(-1, 1)).flatten()])
                for key, value in initial_bids.items()
            )
        )
...
def random_bids_mlca_unif(
    ...
    if fitted_scaler is not None:
        ...
        logging.debug(
            "Random Bids scaled by: %s to the interval [%s,%s]",
            round(fitted_scaler.scale_[0], 8),
            minI,
            maxI,
        )
        random_bids = OrderedDict(
            list(
                (
                    ...
                    [
                        value[0],
                        fitted_scaler.transform(value[1].reshape(-1, 1)).flatten(),
                    ],
                )
                for key, value in random_bids.items()
            )
        )
...
def format_solution_mip_new(Mip, elicited_bids, bidder_names, fitted_scaler):
    ...
    for key in list(S.keys()):
        ...
        if fitted_scaler is not None:
            ...
            logging.debug(value)
            logging.debug(
                "WDP values for allocation scaled by: 1/%s",
                round(fitted_scaler.scale_[0], 8),
            )
...
def unif_random_bids(value_model, bidder_id, n):
    logging.debug(
        "Sampling uniformly at random %s bundle-value pairs from bidder %s",
        n,
        bidder_id,
    )
    ncol = len(value_model.get_good_ids())  # number of items in value model
    D = np.unique(
        np.asarray(random.choices([0, 1], k=n * ncol)).reshape(n, ncol), axis=0
    )
...
def _unif_pseudo_sampling(m, n):
    D = []
    q_init = n
    x = binom.pmf(list(range(m + 1)), m, 1 / 2) * (
        q_init + 1
    ...
    while b < m:
        ...
        for j in range(0, math.floor(max(s, 1))):
            k = random.choices(list(range(a, b + 1)), weights=x[a : b + 1], k=1)[
                0
            ]  # maybe weigh with x
...
def create_value_model(value_model):
    if value_model == "LSVM":
        V = ValueModel(
            ...
            number_of_items=18,
            local_bidder_ids=[],
            regional_bidder_ids=list(range(1, 6)),
            national_bidder_ids=[0],
            scaler=[None],
        ...
        V = ValueModel(
            ...
            number_of_items=18,
            local_bidder_ids=[],
            regional_bidder_ids=list(range(0, 6)),
            national_bidder_ids=[6],
            scaler=[None],
            highFrequency_bidder_ids=[],
        )
        ...
        V = ValueModel(
            name="MRVM",
            number_of_items=98,
            local_bidder_ids=[0, 1, 2],
            regional_bidder_ids=[3, 4, 5, 6],
            national_bidder_ids=[7, 8, 9],
            scaler=[None],
            highFrequency_bidder_ids=[],
        )
        ...
        V = ValueModel(
            name="SRVM",
            number_of_items=29,
            local_bidder_ids=[0, 1],
            highFrequency_bidder_ids=[2],
            regional_bidder_ids=[3, 4],
            national_bidder_ids=[5, 6],
            scaler=[None],
        )
    else:
