...
from mlca.mlca_util import (
    format_solution_mip_new,
    initial_bids_mlca_unif,
    key_to_int,
    timediff_d_h_m_s,
)
...
def eval_bidder_nn(
    ...
    # fit model to data
    train_logs = mvnn.fit(
        epochs=NN_parameters[bidder]["epochs"],
...
class MLCA_Economies:
    def __init__(
        ...
        self.SATS_auction_instance_allocation = (
            None  # true efficient allocation of auction instance
        )
        ...
        self.SATS_auction_instance_seed = (
            SATS_auction_instance_seed  # auction instance seed from SATS
        )
        self.bidder_ids = list(
            SATS_auction_instance.get_bidder_ids()
        )  # bidder ids in this auction instance.
        self.bidder_names = list(
            "Bidder_{}".format(bidder_id) for bidder_id in self.bidder_ids
        )
        self.N = len(self.bidder_ids)  # number of bidders
        self.good_ids = set(
            SATS_auction_instance.get_good_ids()
        )  # good ids in this auction instance
        ...
        self.total_time_elapsed_distr = {"MIP": None, "ML_TRAIN": None, "OTHER": None}
        ...
        subsets = list(
            map(list, itertools.combinations(self.bidder_ids, self.N - 1))
        )  # orderedDict containing all economies and the corresponding bidder ids of the bidders which are active in these economies.
        subsets.sort(
            reverse=True
        )  # sort s.t. Marginal Economy (-0)-> Marginal Economy (-1) ->...
        self.economies = OrderedDict(
            list(
                ("Marginal Economy -({})".format(i), econ)
                for econ, i in zip(
                    subsets,
                    [
                        [x for x in self.bidder_ids if x not in subset][0]
        ...
        self.economies_names = OrderedDict(
            list(
                (key, ["Bidder_{}".format(s) for s in value])
        ...
        self.efficiency_per_iteration = (
            OrderedDict()
        )  # storage for efficiency stat per auction round
        self.efficient_allocation_per_iteration = (
            OrderedDict()
        )  # storage for efficent allocation per auction round
        ...
        self.nn_seed = (
            self.SATS_auction_instance_seed * 10**4
        )  # seed for training the MVNNs, shifted by 10**6 (i.e. one has 10**6 fits capacity until its overlapping with self.nn_seed+1 )
        self.mip_logs = defaultdict(list)
        self.train_logs = {}
        self.total_time_elapsed = None
        self.ml_total_train_time_sec = (
            0  # sums together train_logs thus does not account for parallelization
        )
        self.total_time_elapsed_estimation_step = 0

        # DYNAMIC PER ECONOMY
        self.economy_status = OrderedDict(
            list((key, False) for key, value in self.economies.items())
        )  # boolean, status of economy: if already calculated.
        ...
        )  # Allocation of the WDP based on the elicited bids
        ...
        )  # Social Welfare of the Allocation of the WDP based on the elicited bids
        ...
        self.elapsed_time_mip = OrderedDict(
            list((key, []) for key, value in self.economies.items())
        )  # stored MIP solving times per economy
        self.warm_start_sol = OrderedDict(
            list((key, None) for key, value in self.economies.items())
        )  # MIP SolveSolution object used as warm start per economy
        ...
        self.RS_allocation_cache_for_reusing = OrderedDict(
            list((key, []) for key, value in self.economies.items())
        )
        self.DE_allocation_cache_for_reusing = OrderedDict(
            list((key, []) for key, value in self.economies.items())
        )

        # DYNAMIC PER BIDDER
        self.mlca_payments = OrderedDict(
            list(("Bidder_{}".format(bidder_id), None) for bidder_id in self.bidder_ids)
        )  # VCG-style payments in MLCA, calculated at the end
        self.elicited_bids = OrderedDict(
            list(("Bidder_{}".format(bidder_id), None) for bidder_id in self.bidder_ids)
        )  # R=(R_1,...,R_n) elicited bids per bidder
        self.current_query_profile = OrderedDict(
            list(("Bidder_{}".format(bidder_id), None) for bidder_id in self.bidder_ids)
        )  # S=(S_1,...,S_n) number of actual value queries, that takes into account if the same bundle was queried from a bidder in two different economies that it is not counted twice
        self.NN_parameters = OrderedDict(
            list(("Bidder_{}".format(bidder_id), None) for bidder_id in self.bidder_ids)
        )  # DNNs parameters as in the Class NN described.
        ...
        self.argmax_allocation = OrderedDict(
            list(
                (
                    key,
                    OrderedDict(list((bidder_id, [None, None]) for bidder_id in value)),
                )
                for key, value in self.economies_names.items()
            )
        )  # [a,a_restr] a: argmax bundles per bidder and economy, a_restr: restricted argmax bundles per bidder and economy

        self.sampled_marginals_bidder_counter = OrderedDict(
            list(
                (
                    bidder_id,
                    OrderedDict(
                        list(
                            (key, 0)
                            for key in self.economies_names
                            if key
                            not in ["Main Economy", f"Marginal Economy -({bidder_id})"]
                        )
                    ),
                )
        ...
        if self.separate_economy_training:
            self.NN_models = OrderedDict(
                list(
                    (key, OrderedDict(list((bidder_id, None) for bidder_id in value)))
                    for key, value in self.economies_names.items()
                )
            )
        else:
    ...
    def save_results(self, res_path):
        ...
        save_dict["MLCA Efficiency Marginals"] = (
            self.mlca_efficiency_marginals
        )  # Potentially None for intermediate iteration
        save_dict["MLCA Efficiency per Iteration"] = self.efficiency_per_iteration
        save_dict["MLCA Efficiency per Iteration all Economies"] = (
            self.efficiency_per_iteration_all_economies
        )  # only for new_query_option= 'RS' or 'DE'

        save_dict["MLCA Optimal SCW Main"] = (
            self.mlca_scw,
        )  # Potentially None for intermediate iteration
        save_dict["MLCA Optimal SCW Marginals"] = (
            self.mlca_marginal_scws,
        )  # Potentially None for intermediate iteration
        save_dict["SATS Optimal SCW"] = self.SATS_auction_instance_scw

        save_dict["MLCA Relative Revenue"] = (
            self.relative_revenue,
        )  # None for intermediate iteration
        save_dict["MLCA Payments"] = (
            self.mlca_payments,
        )  # None for intermediate iteration
        ...
        save_dict["MLCA Sampled Marginals per Iteration"] = (
            self.sampled_marginals_per_iteration
        )
        save_dict["MLCA Sampled Marginals Counter"] = self.sampled_marginals_counter
        save_dict["MLCA Sampled Marginals Bidder Counter"] = (
            self.sampled_marginals_bidder_counter
        )
        ...
        save_dict["MLCA Allocation"] = (
            self.mlca_allocation,
        )  # Potentially None for intermediate iteration
        save_dict["SATS Efficient Allocation"] = self.SATS_auction_instance_allocation
        save_dict["MLCA Allocation per Iteration"] = (
            self.efficient_allocation_per_iteration
        )
        ...
        json.dump(
            save_dict,
            open(os.path.join(res_path, "results.json"), "w"),
            indent=4,
            sort_keys=False,
            separators=(", ", ": "),
            ensure_ascii=False,
    ...
    def get_info(self, final_summary=False):
        ...
        if final_summary:
            logging.warning(
                "EFFICIENCY: {} %".format(
                    round(self.mlca_allocation_efficiency, 4) * 100
                )
            )
    ...
    def calc_time_spent(
        ...
        if self.start_time:
            ...
            self.total_time_elapsed_distr["MIP"] = (
                "{}d {}h:{}m:{}s".format(
                    *timediff_d_h_m_s(timedelta(seconds=mip_seconds))
                ),
            ...
            self.total_time_elapsed_distr["ML_TRAIN"] = (
                "{}d {}h:{}m:{}s".format(
                    *timediff_d_h_m_s(timedelta(seconds=ml_estimation_seconds))
                ),
            ...
            self.total_time_elapsed_distr["OTHER"] = (
                "{}d {}h:{}m:{}s".format(
                    *timediff_d_h_m_s(timedelta(seconds=other_seconds))
                ),
            ...
            self.total_time_elapsed = (
                "{}d {}h:{}m:{}s".format(
                    *timediff_d_h_m_s(self.end_time - self.start_time)
                ),
                tot_seconds,
            )
        else:
    ...
    def print_argmax_allocation(self, economy_key):
        logging.info("")
        logging.info(
            f"current ARGMAX ALLOCATION | BIDDER SPECIFIC BUNDLES in {economy_key}:"
        )
        logging.info("-----------------------------------------------")
    ...
    def get_number_of_elicited_bids(self, bidder=None):
        ...
        if bidder is None:
            return OrderedDict(
                (bidder, self.elicited_bids[bidder][0].shape[0] - 1)
                for bidder in self.bidder_names
            )  # -1 since empty-bundle is included
        else:
            return (
                self.elicited_bids[bidder][0].shape[0] - 1
            )  # -1 since empty-bundle is included
    ...
    def calculate_efficiency_per_iteration(self):
        ...
        efficiency = self.calculate_efficiency_of_allocation(
            allocation=allocation, allocation_scw=objective
        )
        logging.debug("Current efficiency: {}".format(efficiency))
        self.efficiency_per_iteration[self.mlca_iteration] = efficiency
        return efficiency

    def set_calc_efficiency_per_iteration_all_economies(self, mlca_iteration: int):
        """
        ...
        if self.new_query_option == "RS" or self.new_query_option == "DE":
            ...
            for k, marginal_scw in self.mlca_marginal_scws.items():
                if marginal_scw:
                    self.efficiency_per_iteration_all_economies[mlca_iteration][k] = (
                        marginal_scw / self.SATS_auction_instance_scw
                    )
                else:
                    self.efficiency_per_iteration_all_economies[mlca_iteration][k] = (
                        None
            ...
            if self.mlca_scw:
                self.efficiency_per_iteration_all_economies[mlca_iteration][
                    "Main Economy"
                ] = self.mlca_scw / self.SATS_auction_instance_scw
            else:
                self.efficiency_per_iteration_all_economies[mlca_iteration][
                    "Main Economy"
                ] = None
    ...
    def set_DE_parameters(self, parameters):

        logging.debug("Set DE parameters")
        self.DE_parameters = parameters

    def set_RS_parameters(self, parameters):

        logging.debug("Set DE parameters")
        self.RS_parameters = parameters

    def set_initial_bids(
        self, initial_bids=None, fitted_scaler=None, seed=None, include_full_bundle=True
    ):
        ...
        if initial_bids is None:  # Uniform sampling (now with correct isLegacy Version)
            self.elicited_bids, self.fitted_scaler = initial_bids_mlca_unif(
                ...
                bidder_names=self.bidder_names,
                scaler=self.scaler,
                seed=seed,
            ...
            self.elicited_bids = (
                initial_bids  # needed format: {Bidder_i:[bundles,values]}
            )
    ...
    def solve_SATS_auction_instance(self):
        if "SATS_auction_instance" in self.__dict__:
            ...
            try:
                ...
                logging.warning(
                    f"JAVA LIBRARY PATH: {System.getProperty('java.library.path')}"
                )
                logging.warning(
                    f"PYTHON LD_LIBRARY_PATH: {os.environ.get('LD_LIBRARY_PATH')}"
                )
            ...
            self.SATS_auction_instance_allocation, self.SATS_auction_instance_scw = (
                self.SATS_auction_instance.get_efficient_allocation()
            )

    def set_global_admissible_marginals(self, mlca_iteration):
        """
        ...
        admissible_marginals = [
            k for k, v in sorted(d_shuffled.items(), key=lambda item: item[1])
        ][: self.Qround]
    ...
    def sample_marginal_economies_for_bidder(self, active_bidder):
        """
        ...
        if self.balanced_global_marginals:
            ...
            admissible_marginals_active_bidder = [
                m
                for m in self.sampled_marginals_per_iteration[self.mlca_iteration]
            ...
            # choice -> select our of the Qround admissible ones the ones which have been selected the least often for active bidder
                ...
                helper_dict = {
                    k: self.sampled_marginals_bidder_counter[active_bidder][k]
                    for k in admissible_marginals_active_bidder
                }
                # Random shuffling
                ...
                sampled_marginals_active_bidder = [
                    k for k, v in sorted(d_shuffled.items(), key=lambda item: item[1])
                ][: (self.Qround - 1)]
            else:
                raise ValueError(
        ...
        # marginals sampled individually for each bidder in each iteration
            ...
            admissible_marginals_active_bidder = [
                x
                for x in list((self.economies.keys()))
                if x
                not in [
                    "Main Economy",
            ...
            sampled_marginals_active_bidder = random.sample(
                admissible_marginals_active_bidder, k=self.Qround - 1
            )
    ...
    def update_elicited_bids(self):
        ...
        for bidder in self.bidder_names:
            ...
            logging.info("---------------------------------------------")
            # update bundles
            self.elicited_bids[bidder][0] = np.append(
                self.elicited_bids[bidder][0],
                self.current_query_profile[bidder],
                axis=0,
            )
            # update values
            bidder_value_reports = self.value_queries(
                bidder_id=key_to_int(bidder), bundles=self.current_query_profile[bidder]
            )
            self.elicited_bids[bidder][1] = np.append(
                self.elicited_bids[bidder][1], bidder_value_reports, axis=0
            )

            logging.info("CHECK Uniqueness of updated elicited bids:")
            check = len(np.unique(self.elicited_bids[bidder][0], axis=0)) == len(
                self.elicited_bids[bidder][0]
            )
            if check:
                logging.info("UNIQUE\n")
    ...
    def value_queries(self, bidder_id, bundles, return_on_original_scale=False):
        ...
        raw_values = np.array(
            [
                self.SATS_auction_instance.calculate_value(bidder_id, bundles[k, :])
                for k in range(bundles.shape[0])
            ]
        )
        if not self.fitted_scaler or return_on_original_scale:
            ...
            minI = int(
                round(self.fitted_scaler.data_min_[0] * self.fitted_scaler.scale_[0])
            )
            maxI = int(
                round(self.fitted_scaler.data_max_[0] * self.fitted_scaler.scale_[0])
            )
            logging.debug("*SCALING*")
            logging.debug("---------------------------------------------")
            logging.debug("raw values %s", raw_values)
            logging.debug(
                "Return value queries scaled by: %s to the interval [%s,%s]",
                round(self.fitted_scaler.scale_[0], 8),
                minI,
                maxI,
            )
            logging.debug(
                "scaled values %s",
                self.fitted_scaler.transform(raw_values.reshape(-1, 1)).flatten(),
            )
            logging.debug("---------------------------------------------")

            return self.fitted_scaler.transform(raw_values.reshape(-1, 1)).flatten()

    def check_bundle_contained(self, bundle, bidder, log_level="INFO"):
        ...
        if self.current_query_profile[bidder] is not None:
            if np.any(np.equal(self.current_query_profile[bidder], bundle).all(axis=1)):
                if log_level == "INFO":
                    logging.info(
                        "Bundle ALREADY QUERIED IN THIS ITERATION from {}".format(
                            bidder
                        )
                    )
                elif log_level == "DEBUG":
    ...
    def check_exp_100_uUB_SCW_is_larger(
        self, allocation, economy_key, precalculated_exp_100_uUB_scw=None
    ):
        ...
        if precalculated_exp_100_uUB_scw is None:
            ...
            exp_100_uUB_scw = self.calculate_exp_100_uUB_scw(
                allocation=allocation, economy_key=economy_key
            )[0]
        else:
        ...
        if exp_100_uUB_scw < current_economy_scw:
            logging.debug(
                "100% SCW OF PROPOSED ALLOCATION IS SMALLER THAN CURRENT SCW IN THIS ECONOMY"
            )
    ...
    def get_current_economy_scw(self, economy_key):
    ...
    def calculate_exp_100_uUB_scw(
        ...
        for bidder, bundle in allocation.items():
            ...
            v *= (
                MODELS[bidder]._target_max / self.local_scaling_factor
            )  # revert local scaling
    ...
    def create_population_check1(
        ...
        num_attempts = 0
        while (
            cur_population_size < desired_population_size
            and num_attempts < max_init_attempts
        ):
            num_attempts += 1
            ...
            # --------------------------------------------------------------
            if self.check_bundle_contained(
                bundle=random_allocation[active_bidder],
                bidder=active_bidder,
                log_level="DEBUG",
            ):
                self.new_query_DE_STATS[self.mlca_iteration][economy_key][
                    active_bidder
                ]["failed_CHECK1_init_pop"] += 1
                continue
            # --------------------------------------------------------------
        ...
        if cur_population_size == desired_population_size:
            ...
            logging.info(
                "population check1 unsuccessfull, but reached at least desired_population_size/2."
            )
    ...
    def create_initial_DE_population(
        self,
        desired_population_size,
        max_init_attempts,
        economy_key,
        ...
        while (
            cur_population_size < desired_population_size
            and num_eval < max_init_attempts
        ):
            num_batches += 1
            remaining_eval_budget = max_init_attempts - num_eval
            if (
                remaining_eval_budget
                < desired_population_size * 0.5 - cur_population_size
            ):
                logging.info(
                    "Initial DE population unsuccessfull, solving the restricted MIP."
                )
                return None, None
            eval_batch_size = max(
                64,
                min(
                    remaining_eval_budget,
                    int(
                        (desired_population_size - cur_population_size)
                        * batching_factor
                    )
                    + 1,
                ),
            ...
            exp_100_uUB_scw = self.calculate_exp_100_uUB_scw(
                allocation=population_check1, economy_key=economy_key
            )
            ...
            if (
                num_batches == 1
                and success_rate_check_2 * max_init_attempts
                < 0.6 * desired_population_size
            ):
                logging.info(
                    "Initial DE population unsuccessfull, solving the restricted MIP."
                )
        ...
        if cur_population_size >= desired_population_size:
            ...
            logging.info(
                "Initial DE population unsuccessfull, but reached at least desired_population_size/2."
            )
            population_size = cur_population_size
            return population, population_size
    ...
    def get_new_DE_bundle(self, economy_key, active_bidder, iterations=10000):
        ...
        if not self.new_query_DE_STATS[self.mlca_iteration].get(economy_key):
            self.new_query_DE_STATS[self.mlca_iteration][economy_key] = {}
        if not self.new_query_DE_STATS[self.mlca_iteration][economy_key].get(
            active_bidder
        ):
        ...
        for prev_allocation in self.DE_allocation_cache_for_reusing[economy_key]:
            ...
            logging.debug(
                f"{active_bidder} BUNDLE:{prev_allocation[active_bidder].reshape(-1)}"
            )
            # CHECK 1 (only needed here since CHECK 2 has already been performed): IF ACTIVE BIDDER BUNDLE HAS BEEN ALREADY ELICITED
            # --------------------------------------------------------------
            if not self.check_bundle_contained(
                bundle=prev_allocation[active_bidder],
                bidder=active_bidder,
                log_level="DEBUG",
            ):
                logging.info(
                    "REUSE BUNDLE FROM A PREVIOUS DE ALLOC IN THIS ECONOMY AND ITERATION"
                )
                self.new_query_DE_STATS[self.mlca_iteration][economy_key][
                    active_bidder
                ]["reused"] = True
        ...
        def convert_to_item_bidder_matrix(
            individual_idx: int, population: dict, num_items: int
        ):
            item_bidder_matrix = np.zeros(shape=(len(population), num_items))
            for bidder_idx, bidder in enumerate(
        ...
        population, population_size = self.create_initial_DE_population(
        ...
        for i in range(1, iterations + 1):
            # DE Step 1: SELECTION
            # ----------------------------------------------------------------------------------------------------------------------------
            uUB_scw = self.calculate_uUB_scw(
                allocation=trial_population, economy_key=economy_key
            )
            exp_100_uUB_scw = self.calculate_exp_100_uUB_scw(
                allocation=trial_population, economy_key=economy_key
            )
            trial_population_scw = (
                self.DE_parameters["weight_uUB_acquisition"] * uUB_scw
            ...
            for population_idx in range(population_size):
                ...
                if (
                    trial_population_scw[population_idx]
                    < population_scw[population_idx]
                ):
                    self.new_query_DE_STATS[self.mlca_iteration][economy_key][
                        active_bidder
                    ]["failed_CHECK_SCW"] += 1
                ...
                # --------------------------------------------------------------
                if self.check_bundle_contained(
                    bundle=trial_population[active_bidder][population_idx],
                    bidder=active_bidder,
                    log_level="DEBUG",
                ):
                    self.new_query_DE_STATS[self.mlca_iteration][economy_key][
                ...
                # CHECK 2: IF 100% SCW OF PROPOSED RANDOM ALLOCATION IS LARGER OR EQUAL TO CURRENT EFFICIENCY IN ECONOMY
                # --------------------------------------------------------------
                if not self.check_exp_100_uUB_SCW_is_larger(
                    allocation=None,
                    economy_key=economy_key,
                    precalculated_exp_100_uUB_scw=exp_100_uUB_scw[population_idx],
                )[0]:
                    self.new_query_DE_STATS[self.mlca_iteration][economy_key][
                        active_bidder
                    ]["failed_CHECK2"] += 1
                ...
                for bidder in trial_population.keys():
                    new_population[bidder][population_idx] = trial_population[bidder][
                        population_idx
                    ]
                    new_population_scw[population_idx] = trial_population_scw[
                        population_idx
                    ]
            logging.info("SELECTION END")
            incumbent_scw, incumbent_idx = (
                max(new_population_scw),
                np.argmax(new_population_scw),
            )
            incumbent_allocation = {
                bidder: bundles[incumbent_idx]
                for bidder, bundles in new_population.items()
            }
            logging.info(
                "DE Iteration: {} | Current Incumbent SCW: {:.3f} | True Economy SCW: {:.3f} | Changed Indiv.: {}".format(
                    i,
                    incumbent_scw,
                    self.get_current_economy_scw(economy_key),
                    changed_individuals,
                )
            )
            self.new_query_DE_STATS[self.mlca_iteration][economy_key][active_bidder][
            ...
            # ----------------------------------------------------------------------------------------------------------------------------
            convert_to_mat = partial(
                convert_to_item_bidder_matrix, population=population, num_items=self.M
            )
            if mutation_strategy == "rand_1":
                ...
                def mutation_func(population_idx, population_size, incumbent_idx):
                    ...
                    x_1_idx, x_2_idx = np.random.choice(
                        sampling_index, size=2, replace=False, p=p_pos
                    )
                    ...
                    (x_3_idx,) = np.random.choice(
                        sampling_index, size=1, replace=False, p=p_neg
                    )
                    return convert_to_mat(
                        x_1_idx
                    ) + random.random() * mutation_strength * (
                ...
                raise NotImplementedError(
                    "Unknown mutation strategy {}".format(mutation_strategy)
                )
            # ----------------------------------------------------------------------------------------------------------------------------
            logging.info("MUTATION - START")
            mutation_func_wrapped = globalize(
                partial(
                    mutation_func,
            ...
            if reprojection_strategy == "round-clip":
                ...
                bidder_sampling_matrix = np.array(
                    torch.nn.functional.softmax(
                        torch.from_numpy(mutants / softmax_temperature), dim=1
                    )
                )
                mutants = parallel_apply_along_axis(
                    globalize(lambda pval: np.random.multinomial(1, pval)),
                    1,
                    bidder_sampling_matrix,
                    num_cpus=num_cpus,
                )
                ...
                raise NotImplementedError(
                    "Unknown reprojection strategy {}".format(reprojection_strategy)
                )
            # ----------------------------------------------------------------------------------------------------------------------------
            ...
            if crossover_strategy == "bin":
                ...
                raise NotImplementedError(
                    "Unknown crossover strategy {}".format(crossover_strategy)
                )
            # ----------------------------------------------------------------------------------------------------------------------------
        ...
        return (
            incumbent_allocation[active_bidder]
            .reshape(
                -1,
            )
            .astype(int)
        )  # reshape such that fits format of update_current_query_profile

    def get_random_allocation(self, economy_key):
        """
        ...
        random_idx = dict(
            list((bidder, []) for bidder in self.economies_names[economy_key])
        )
    ...
    def get_new_RS_bundle(self, economy_key, active_bidder, attempts=10000):
        ...
        if not self.new_query_RS_STATS[self.mlca_iteration][economy_key].get(
            active_bidder
        ):
        ...
        for prev_allocation in self.RS_allocation_cache_for_reusing[economy_key]:
            ...
            if not self.check_bundle_contained(
                ...
                logging.info(
                    "REUSE BUNDLE FROM A PREVIOUS RS ALLOC IN THIS ECONOMY AND ITERATION"
                )
                self.new_query_RS_STATS[self.mlca_iteration][economy_key][
                    active_bidder
                ]["reused"] = True
        ...
        for i in range(1, attempts + 1):
            ...
            # CHECK 2: IF 100% SCW OF PROPOSED RANDOM ALLOCATION IS LARGER OR EQUAL TO CURRENT EFFICIENCY IN ECONOMY
            # --------------------------------------------------------------
            CHECK2, exp_100_uUB_scw, true_scw = self.check_exp_100_uUB_SCW_is_larger(
            ...
            # --------------------------------------------------------------
            uUB_scw = self.calculate_uUB_scw(
                allocation=random_allocation, economy_key=economy_key
            )[0]
            acquisition_scw = (
                self.RS_parameters["weight_uUB_acquisition"] * uUB_scw
        ...
        if incumbent_allocation:
            ...
            for k, v in self.new_query_RS_STATS[self.mlca_iteration][economy_key][
                ...
                logging.info(f"->{k}:{v}")
            self.RS_allocation_cache_for_reusing[economy_key].append(
                incumbent_allocation
            )
    ...
    def next_queries(self, economy_key, active_bidder):
        ...
        if not self.economy_status[economy_key]:
            ...
            # ----------------------------------------------------------------------------
            if (
                self.separate_economy_training
            ):  # only fit here if separate_economy_training=True
        ...
        if self.check_bundle_contained(
            bundle=self.argmax_allocation[economy_key][active_bidder][0],
            bidder=active_bidder,
        ):
            if self.current_query_profile[active_bidder] is not None:
                Ri_union_Si = np.append(
                    self.elicited_bids[active_bidder][0],
                    self.current_query_profile[active_bidder],
                    axis=0,
                )
            else:
            ...
            return self.argmax_allocation[economy_key][active_bidder][
                1
            ]  # return constrained argmax bundle
        ...
        # NEW BUNDLE HAS NOT BEEN ALREADY QUERIED: accept it
            ...
            return self.argmax_allocation[economy_key][active_bidder][
                0
            ]  # return regular argmax bundle

    def estimation_step_economy(self, economy_key):

        start_estimation = datetime.now()
        logging.info(f"ESTIMATION STEP:{economy_key}")
        logging.info("-----------------------------------------------")

        training_seeds = self.update_nn_training_seeds(
            number_of_seeds=len(self.economies_names[economy_key])
        )
        ...
        if not self.train_logs.get(self.mlca_iteration):
            self.train_logs[self.mlca_iteration] = OrderedDict(
                list(
                    (key, OrderedDict(list((bidder_id, []) for bidder_id in value)))
                    for key, value in self.economies_names.items()
                )
            )
        ...
        self.total_time_elapsed_estimation_step += (
            end_estimation - start_estimation
        ).total_seconds()
        logging.info(
            "Elapsed Time: {}d {}h:{}m:{}s\n".format(
                *timediff_d_h_m_s(end_estimation - start_estimation)
            )
        )
        return
    ...
    def estimation_step(self):
        ...
        training_seeds = self.update_nn_training_seeds(
            number_of_seeds=len(self.bidder_names)
        )
    ...
    def update_nn_training_seeds(self, number_of_seeds):
    ...
    def get_ML_models(self, economy_key, model_type):
        """
        ...
        if model_type == "uUB_model":
            for bidder in self.economies_names[economy_key]:
                if self.separate_economy_training:
                    assert self.economies_names[economy_key] == list(
                        self.NN_models[economy_key].keys()
                    )
            ...
            for bidder in self.economies_names[economy_key]:
                if self.separate_economy_training:
                    ...
                    MODELS[bidder] = self.NN_models[economy_key][
                        bidder
                    ].exp_100_uUB_model
                else:
    ...
    def calculate_mlca_allocation(self, economy="Main Economy"):
        ...
        allocation, objective = self.solve_WDP(
            elicited_bids=OrderedDict(
                list((k, self.elicited_bids.get(k, None)) for k in active_bidders)
            )
        )
    ...
    def solve_WDP(self, elicited_bids, verbose=0):
        ...
        if verbose == 1:
            logging.debug(
                "Solving WDP based on elicited bids for bidder: %s", bidder_names
            )
        elicited_bundle_value_pairs = [
            np.concatenate((bids[0], np.asarray(bids[1]).reshape(-1, 1)), axis=1)
            for bidder, bids in elicited_bids.items()
        ...
        if self.fitted_scaler is not None:
            if verbose == 1:
                ...
                logging.debug(
                    "WDP objective value scaled by: 1/%s",
                    round(self.fitted_scaler.scale_[0], 8),
                )
    ...
    def calculate_vcg_payments(self, forced_recalc=False):
        ...
        for bidder in self.bidder_names:
            marginal_economy_bidder = "Marginal Economy -({})".format(
                key_to_int(bidder)
            )
            p1 = self.mlca_marginal_scws[
                marginal_economy_bidder
            ]  # social welfare of the allocation a^(-i) in this economy
            p2 = sum(
                [
                    self.mlca_allocation[i]["value"]
                    for i in self.economies_names[marginal_economy_bidder]
                ]
            )  # social welfare of mlca allocation without bidder i
            self.mlca_payments[bidder] = round(p1 - p2, 2)
            logging.info(
                "Payment %s: %s - %s  =  %s", bidder, p1, p2, self.mlca_payments[bidder]
            )
        ...
        logging.info(
            "Revenue: {} | {}% of SCW in efficient allocation\n".format(
                self.revenue, self.relative_revenue * 100
            )
        )
