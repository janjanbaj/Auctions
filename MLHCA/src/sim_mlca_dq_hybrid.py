...
# 1. Force the environment variable inside Python before any imports
lib_path = "/home/janeet/Code/MLHCA/src/lib"
os.environ["PYJNIUS_CLASSPATH"] = lib_path

# 2. Configure jnius_config (this satisfies the JVM startup)
import jnius_config

if not jnius_config.vm_running:
    jnius_config.add_classpath(
        os.path.join(lib_path, "sats-0.8.2-SNAPSHOT-jar-with-dependencies.jar")
    )
    jnius_config.add_classpath(os.path.join(lib_path, "cplex.jar"))
...
# 4. Critical: Import PySats and initialize it BEFORE other local modules
# This prevents the 'VM already running' error in sub-modules
from pysats import PySats

try:
    PySats.getInstance()
except ValueError:
    # If it still complains about the VM already running, we ignore it
    # because we manually set the classpath above.
    pass

# 5. Now import the rest of your local project modules
...
CPLEXUtils.SOLVER.setSolveParam(
    SolveParams.THREADS, 8
)  # TODO: Change the number of threads to be the same as in the script.
# --------------------------------------
...
def main(
    ...
    # Ignore Gurobi's Warnings of Small
    import warnings
    ...
    # 1. SATS Parameters
    # ----------------------------------------------
    SATS_parameters = {"SATS_domain": domain, "isLegacy": False, "SATS_seed": seed}
    # ----------------------------------------------

    # 1.5 Load the json with the HPOed parameters for the MVNNs and the one for the linear prices
    hpo_file_name = "hpo_configs_hybrid.json"  # TODO: change back
    hpo_dict = json.load(open(hpo_file_name, "r"))
    hpo_file_name_dqs = "hpo_configs_hybrid.json"  # the HPO file name for dqs
    ...
    price_dict = json.load(
        open(f"{domain}_{price_file_name}.json", "r")
    )  # AVG value per item
    all_bidders_scales = {}
    ...
    for i in range(num_bidders):
        ...
        # ----------------------------------------------
        MVNN_parameters = {
            "num_hidden_layers": hpo_dict[domain][bidder_type]["num_hidden_layers"],
        ...
        MVNN_parameters_dqs = {
            "num_hidden_layers": hpo_dict_dqs[domain][bidder_type]["num_hidden_layers"],
            "num_hidden_units": hpo_dict_dqs[domain][bidder_type]["num_hidden_units"],
            "layer_type": "MVNNLayerReLUProjected",
            "target_max": 1,
            "lin_skip_connection": hpo_dict_dqs[domain][bidder_type][
                "lin_skip_connection"
            ],
            "dropout_prob": 0,
            "init_method": "custom",
        ...
        if domain == "GSVM":
            ...
            end_linear_item_prices_multiplier = (
                40  # TODO: FIX if we implement this method
            )
        elif domain == "SRVM":
            end_linear_item_prices_multiplier = (
                40  # TODO: FIX if we implement this method
        ...
        TRAIN_parameters = {
            ...
            "max_linear_prices_multiplier": hpo_dict[domain][bidder_type][
                "max_linear_prices_multiplier"
            ],  # NOTE: only used for "initial_demand_query_method=random"; they actually get loaded from the file
            "max_linear_price": all_bidders_max_linear_prices[
                i
            ],  # NOTE: only used for "initial_demand_query_method=random"; they actually get loaded from the file
            "scale": all_bidders_scales[
                i
            ],  # NOTE: they actually get loaded from the file
            "start_linear_item_prices": np.zeros(
                num_items
            ),  # NOTE: only used for "initial_demand_query_method=increasing"
            "end_linear_item_prices": np.ones(num_items)
            * end_linear_item_prices_multiplier,  # NOTE: only used for "initial_demand_query_method=increasing"
            ...
            "epochs": hpo_dict[domain][bidder_type][
                "epochs"
            ],  # TODO: Change back to original!!!
            "cached_solution_frequency": hpo_dict[domain][bidder_type][
            ...
            "clip_grad_norm": 1,
            "use_gradient_clipping": hpo_dict[domain][bidder_type][
                "use_gradient_clipping"
            ],
            "scale_multiplier": hpo_dict[domain][bidder_type].get(
                "scale_multiplier", 1
            ),  # NOTE: only used for dynamic scaling.
            "print_frequency": 1,
            "generic_approach": True,  # NOTE: should be true for the main table results
        }

        TRAIN_parameters_dqs = {
            "number_val_data_points": 10,
            "max_linear_prices_multiplier": hpo_dict_dqs[domain][bidder_type][
                "max_linear_prices_multiplier"
            ],  # NOTE: only used for "initial_demand_query_method=random"; they actually get loaded from the file
            "max_linear_price": all_bidders_max_linear_prices[
                i
            ...
            * end_linear_item_prices_multiplier,  # NOTE: only used for "initial_demand_query_method=increasing"
            ...
            "epochs": hpo_dict_dqs[domain][bidder_type][
                "epochs"
            ],  # TODO: Change back to original!!!
            ...
            "scale_multiplier": hpo_dict_dqs[domain][bidder_type].get(
                "scale_multiplier", 1
            ),  # NOTE: only used for dynamic scaling.
            "print_frequency": 1,
            "generic_approach": True,  # NOTE: should be true for the main table results
    ...
    mechanism_parameters = {
        ...
        "cca_start_linear_item_prices": np.load(
            f"{domain}_{average_price_file_name}.npy"
        ),  # NOTE: only used for "initial_demand_query_method=cca"
        "cca_initial_prices_multiplier": 0.2
        if domain in ["LSVM", "MRVM"]
        else 1.6,  # NOTE: only used for "initial_demand_query_method=cca", will multiply the initial prices.
        ...
        else True,  # only true for MRVM
        "hpo_file_name": hpo_file_name,
        "W_epochs": 100
        if domain in ["GSVM", "LSVM"]
        else 300,  # NOTE: used to be 1000 for other domains for ML-CCA
        "W_lr": 1
        if domain in ["GSVM", "LSVM"]
        else 4
        if domain in ["SRVM"]
        ...
        "W_v2_max_steps_without_improvement": 100
        if domain in ["GSVM", "LSVM", "SRVM"]
        else 300,  # parameters for the new GD procedure to minimize W
        "W_v2_lr": 1
        if domain in ["GSVM", "LSVM"]
        else 4
        if domain in ["SRVM"]
    ...
    elif mechanism_parameters["new_query_option"] in [
        "gd_linear_prices_on_W",
        "gd_linear_prices_on_W_v2",
        "gd_linear_prices_on_W_v3",
    ]:
        if domain in ["LSVM"]:
    ...
    if mechanism_parameters["new_query_option"] == "cca":
        if mechanism_parameters["Qmax"] == 100:
            ...
            mechanism_parameters["cca_increment"] = (
                0.1025  # so that after 50 clock rounds we have the same possible price increments as in the 100 clock round case.
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--domain", help="name of the domain to run", default="SRVM", type=str
    )
    parser.add_argument(
        "--init_method",
        help="method for the Qinit queries, options: random, increasing, cca",
        default="cca",
        type=str,
    )
    parser.add_argument(
        "--cca_dq", help="number of initial queries", default=20, type=int
    )
    parser.add_argument(
        "--mlcca_dq", help="number of demand queries", default=20, type=int
    )  # TODO: Change back to 20
    parser.add_argument(
        "--mlcca_vq", help="number of value queries", default=60, type=int
    )  # TODO: Change back to 60
    parser.add_argument(
        "--seed", help="auction instance seed to run", default=184, type=int
    )
    parser.add_argument(
        "--new_query_option",
        help="new query option",
        default="gd_linear_prices_on_W_v3",
        type=str,
    )  # options: gd_linear_prices_on_W, gd_linear_prices_on_W_v2, gd_linear_prices_on_W_v3, cca and gd_linear_prices_on_W_v3_cheating
    parser.add_argument(
        "--forbid_individual_bundles",
        help="forbid individual bundles",
        default="true",
        type=str,
    )
    ...
    print(
        f"Starting instance on domain: {domain} with init method: {init_method} cca_dq: {cca_dq} mlcca_dq: {mlcca_dq} mlcca_vq: {mlcca_vq} seed: {seed} new_query_option: {new_query_option}"
    )
    ...
    main(
        ...
        mlcca_dq=mlcca_dq,
        mlcca_vq=mlcca_vq,
        forbid_individual_bundles=forbid_individual_bundles,
        seed=seed,
    )
