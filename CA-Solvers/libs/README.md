...
For the standard CATS model, it is the case that using the ```sh -cplex``` flag, we generate a CPLEX compatible ```*.lp``` file. I currently use PuLP in Python for solving LPs so the following method helps me convert ```sh *.lp -> *.mps```.
